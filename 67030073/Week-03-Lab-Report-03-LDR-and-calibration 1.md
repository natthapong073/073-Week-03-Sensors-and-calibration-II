...
| สภาพแสง  | ความต้านทาน   | ค่าที่วัดได้ | แรงดันเอาต์พุต (โวลต์) |
| -------- | ------------- | ------------ | ---------------------- |
| มืดสนิท  | 200 kΩ – 2 MΩ | 383.35 kΩ    | 383.35 V               |
| ห้องปกติ | 10 kΩ – 50 kΩ | 4.957 kΩ     | 4.957 V                |
| ใต้โคมไฟ | 2 kΩ – 10 kΩ  | 163.3 Ω      | 0.1633 V               |
| กลางแจ้ง | 200 Ω – 2 kΩ  | -            | -                      |
...
<img width="682" height="412" alt="image" src="https://github.com/user-attachments/assets/b15705af-7ab8-458d-966e-df30b85a5dff" />
...
1. ทำไมต้องใช้ Voltage Divider ในการอ่าน LDR? เพราะบอร์ดอ่านความต้านทานตรงๆ ไม่ได้ครับ เลยต้องต่อวงจรเพื่อแปลงให้เป็นแรงดันไฟฟ้า (โวลต์) ก่อน ขา ADC ถึงจะอ่านได้
2. ทำไมต้องเก็บข้อมูลหลายจุดก่อนทำ Calibration? เพราะค่าเซนเซอร์ LDR กับแสงมันไม่ได้แปรผันตรงแบบเส้นตรงครับ ต้องเก็บหลายๆ จุดเพื่อเอาไปสร้างกราฟและสมการให้แม่นยำที่สุด
3. ถ้า Noise มากกว่าสัญญาณจริง จะเกิดอะไรขึ้น? ค่าที่อ่านได้จะแกว่งและมั่วไปหมดครับ เพราะบอร์ดจะแยกไม่ออกเลยว่าอันไหนคือค่าแสงจริงๆ อันไหนคือสัญญาณรบกวน
4. ทำไมการวางตำแหน่งเซนเซอร์จึงสำคัญต่อ Signal Integrity? ถ้าต่อสายยาวไปหรือวางมั่ว สายไฟจะกลายเป็นเสาอากาศดูดสัญญาณรบกวน (Noise) เข้ามาผสม ทำให้แรงดันที่ส่งไปหาบอร์ดเพี้ยนครับ
5. ถ้า ADC มี Resolution ต่ำ จะส่งผลอย่างไรต่อความแม่นยำ? มันจะแบ่งช่วงการอ่านค่าได้ "หยาบ" ครับ เวลาแสงเปลี่ยนไปแค่สว่างขึ้นหรือมืดลงนิดเดียว บอร์ดก็จะไม่รู้เรื่อง ทำให้ค่าที่ได้ไม่ละเอียดครับ
...
| --- | -------------------------------------------------------------- | -------------------- | ---------------------------------------------- |
| 1.1 | อธิบายหลักการทำงานของ LDR ได้ถูกต้อง                           | 5                    | เข้าใจว่าความต้านทานลดลงเมื่อแสงเพิ่ม          |
| 1.2 | อธิบายการทำงานภายใน LDR (โฟตอน → อิเล็กตรอน → conduction band) | 4                    | พออธิบายเรื่องอิเล็กตรอนข้าม Bandgap ได้       |
| 1.3 | เข้าใจว่าความสัมพันธ์ R vs Lux ไม่เป็นเส้นตรง                  | 5                    | เข้าใจว่ากราฟความต้านทานเป็นเส้นโค้ง           |
| 1.4 | เข้าใจเหตุผลที่ต้องใช้ Voltage Divider                         | 5                    | อ่าน R ตรงๆ ไม่ได้ ต้องแปลงเป็นแรงดัน (V) ก่อน |

### **ส่วนที่ 10.2 — การต่อวงจรและอ่านค่า ADC (20 คะแนน)**

| ข้อ | รายการ                              | คะแนน<br>(0–5 คะแนน) | เหตุผล                                     |
| --- | ----------------------------------- | -------------------- | ------------------------------------------ |
| 2.1 | ต่อวงจร Voltage Divider ได้ถูกต้อง  | 5                    | ต่อวงจรและเสียบเข้าขาบอร์ดถูก              |
| 2.2 | เลือกค่า R คงที่เหมาะสม (เช่น 10kΩ) | 5                    | เลือกใช้ 10kΩ ซึ่งเหมาะกับสภาวะแสงในห้อง   |
| 2.3 | เขียนโปรแกรมอ่านค่า ADC ได้ถูกต้อง  | 5                    | เขียนโค้ดอ่านค่าโชว์ใน Serial Monitor ได้  |
| 2.4 | ใช้ ESP‑IDF ADC + Calibration ได้   | 4                    | ตั้งค่า ADC ให้รับไฟ 3.3V ของบอร์ด ESP ได้ |

### **ส่วนที่ 10.3 — การเก็บข้อมูลจริง (20 คะแนน)**

| ข้อ | รายการ                             | คะแนน<br>(0–5 คะแนน) | เหตุผล                                         |
| --- | ---------------------------------- | -------------------- | ---------------------------------------------- |
| 3.1 | เก็บข้อมูล ADC อย่างน้อย 5–7 จุด   | 5                    | เก็บข้อมูลครบทั้ง 7 ระดับแสง                   |
| 3.2 | วัดค่า Lux จากมือถือได้ถูกต้อง     | 4                    | ใช้แอปมือถือวัดค่า Lux เทียบได้                |
| 3.3 | จดข้อมูลเป็นตารางครบถ้วน           | 5                    | จดบันทึกข้อมูลลงตารางเรียบร้อย                 |
| 3.4 | ควบคุมสภาพแสงให้คงที่ระหว่างการวัด | 4                    | พยายามคุมแสงแล้ว แต่ยังมีแสงรอบข้างกวนนิดหน่อย |
...
| ข้อ | รายการ                                        | คะแนน<br>(0–5 คะแนน) | เหตุผล                                         |
| --- | --------------------------------------------- | -------------------- | ---------------------------------------------- |
| 4.1 | สร้างกราฟ Scatter ADC vs Lux ได้              | 5                    | พล็อตจุดลงกราฟใน Excel ได้                     |
| 4.2 | เลือก Trendline เหมาะสม (Linear / Polynomial) | 5                    | เลือก Trendline แบบยกกำลัง (Power) ได้โค้งพอดี |
| 4.3 | อ่านสมการจากกราฟได้ถูกต้อง                    | 5                    | ดึงสมการจากกราฟมาใช้งานได้                     |
| 4.4 | นำสมการไปใช้ในโค้ดได้                         | 4                    | แปลงสมการไปเขียนลงในโค้ดได้จริง                |
| 4.5 | วิเคราะห์ว่า Linear หรือ Polynomial เหมาะกว่า | 4                    | เข้าใจว่าสมการเส้นโค้งแม่นกว่าเส้นตรง          |
### **ส่วนที่ 10.5 — การทดสอบผลลัพธ์และวิเคราะห์ (15 คะแนน)**

| ข้อ | รายการ                                   | คะแนน<br>(0–5 คะแนน) | เหตุผล                                          |
| --- | ---------------------------------------- | -------------------- | ----------------------------------------------- |
| 5.1 | ทดสอบจุดแสงใหม่ที่ไม่ได้ใช้ตอนเก็บข้อมูล | 5                    | ลองเทสกับแสงไฟฉายระดับอื่นๆ แล้ว                |
| 5.2 | เปรียบเทียบ Lux มือถือ vs Lux จากโค้ด    | 4                    | ค่าใกล้เคียงกัน แต่ตรงที่สว่างมากๆ มีเพี้ยนบ้าง |
| 5.3 | วิเคราะห์ความคลาดเคลื่อนและสาเหตุได้     | 4                    | รู้ว่าที่เพี้ยนเพราะมุมเซนเซอร์รับแสงไม่เท่ากัน |
.................................................................
...
1. จุดไหนในใบงานที่คิดว่ายากที่สุด? การพยายามอ่านค่าแสงให้ออกมาแม่นยำและนิ่งครับ เพราะเซนเซอร์ LDR ไวต่อแสงรอบข้างมาก แค่มุมแสงเปลี่ยนหรือมีเงาบังนิดเดียว ค่าที่อ่านได้เทียบกับเซนเซอร์มือถือก็เพี้ยนแล้ว ทำให้หาค่าแสงที่แน่นอนได้ค่อนข้างยากครับ
    
2. ถ้าทำใบงานนี้อีกครั้ง จะปรับปรุงอะไร? การคุมแสงตอนเก็บข้อมูลครับ รอบหน้าจะหากล่องทึบมาครอบเซนเซอร์กับมือถือตอนวัด แสงรบกวนจะได้ไม่เยอะ ค่าที่ได้จะแม่นขึ้น
    
3. ส่วนไหนที่คิดว่าตนเองเข้าใจดีมาก? การต่อวงจร Voltage Divider ครับ เข้าใจภาพชัดเจนเลยว่าเราต้องแปลงความต้านทานเป็นแรงดันไฟฟ้า(โวลต์)ก่อน บอร์ดถึงจะอ่านค่าได้
